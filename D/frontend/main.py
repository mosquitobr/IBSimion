...
TRANSLATIONS = {
    "PT": {
        ...
        "win_title": "IBSimion v2.0.1 - Core Pipeline Estável",
        "menu_file": "Arquivo",
        ...
        "console_box": "Logs do Kernel Linux no WSL2",
    ...
    "EN": {
        ...
        "win_title": "IBSimion v2.0.1 - Core Pipeline Stable",
        "menu_file": "File",
        ...
        "console_box": "Linux Kernel Logs in WSL2",
        ...
        if self.language == "PT":
            ...
            self.manual_chapters = {
                ...
                "Nelder-Mead Optimizer": """
                    ...
                    <ul>
                        <li><b>Optimization Process:</b> The algorithm iteratively adjusts the voltages of all Dirichlet electrodes. At each step, it runs the physical simulation in WSL, reads the <code>tof.txt</code> output, and calculates the Loss function.</li>
...
class SimWorker(QThread):
    ...
    def run(self):
        ...
        if not self.is_opt:
            self.status_signal.emit("Executando simulação física no WSL..." if self.language == "PT" else "Running physical simulation in WSL...")
    ...
    def execute_wsl(self, wsl_dir):
        ...
        try:
            ...
            while True:
                if not self.running:
                    try:
                        subprocess.run("wsl killall ibsimu_wrapper", shell=True, creationflags=subprocess.CREATE_NO_WINDOW if os.name == 'nt' else 0)
                    except Exception:
                        pass
...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        """)
        self.menu_bar.setCornerWidget(self.combo_lang, Qt.TopRightCorner)
    ...
    def create_run_tab(self):
        ...
        # Console Log Console
        self.console_box = QGroupBox("Logs do Kernel Linux / WSL2")
    ...
    def on_opt_step(self, iter_idx, max_iter, loss, voltages):
        ...
        self.canvas_convergence.axes.set_xlabel(self.tr("plot_convergence_xlabel"), color='#F3F4F6')
        self.canvas_convergence.axes.set_ylabel(self.tr("plot_convergence_ylabel"), color='#F3F4F6')
        self.canvas_convergence.axes.set_title(self.tr("plot_convergence_title"), color='#3B82F6')
    ...
    def calculate_diagnostics_plots(self):
        ...
        if data is None or data.size == 0:
            ...
            self.canvas_tof.axes.text(0.5, 0.5, "Aguardando dados da simulação..." if self.language == "PT" else "Waiting for simulation data...",
                                      ha='center', va='center', color='#9CA3AF', transform=self.canvas_tof.axes.transAxes, fontsize=12)
            ...
            self.canvas_emittance.axes.text(0.5, 0.5, "Aguardando dados da simulação..." if self.language == "PT" else "Waiting for simulation data...",
                                            ha='center', va='center', color='#9CA3AF', transform=self.canvas_emittance.axes.transAxes, fontsize=12)
            ...
            self.canvas_contours.axes.text(0.5, 0.5, "Aguardando dados da simulação..." if self.language == "PT" else "Waiting for simulation data...",
                                           ha='center', va='center', color='#9CA3AF', transform=self.canvas_contours.axes.transAxes, fontsize=12)
        ...
        try:
            ...
            if data.size > 0:
                ...
                self.canvas_tof.axes.set_xlabel(self.tr("plot_tof_xlabel"), color='#F3F4F6')
                self.canvas_tof.axes.set_ylabel(self.tr("plot_tof_ylabel"), color='#F3F4F6')
                self.canvas_tof.axes.set_title(self.tr("plot_tof_title"), color='#3B82F6')
                ...
                self.canvas_profile.axes.set_xlabel(self.tr("plot_profile_xlabel"), color='#F3F4F6')
                self.canvas_profile.axes.set_ylabel(self.tr("plot_profile_ylabel"), color='#F3F4F6')
                self.canvas_profile.axes.set_title(self.tr("plot_profile_title"), color='#3B82F6')
                ...
                for ax in [ax_x, ax_y]:
                    ...
                    ax.tick_params(colors='#F3F4F6', which='both')
                ...
                self.canvas_emittance.axes.set_xlabel("Single-particle Emittance (mm·mrad)" if self.language == "EN" else "Emitância Individual (mm·mrad)", color='#F3F4F6')
                self.canvas_emittance.axes.set_ylabel("Count" if self.language == "EN" else "Contagem", color='#F3F4F6')
                self.canvas_emittance.axes.set_title("Emittance Distribution" if self.language == "EN" else "Distribuição de Emitância", color='#3B82F6')
                self.canvas_emittance.axes.grid(True, color='#1F2937')
                self.canvas_emittance.draw()
        ...
        if zs is not None and xs is not None:
            try:
                ...
                self.canvas_contours.axes.spines['bottom'].set_color('#1F2937')
                self.canvas_contours.axes.spines['top'].set_color('#1F2937')
                self.canvas_contours.axes.spines['left'].set_color('#1F2937')
                self.canvas_contours.axes.spines['right'].set_color('#1F2937')
                self.canvas_contours.axes.tick_params(colors='#F3F4F6', which='both')
                self.canvas_contours.axes.xaxis.label.set_color('#F3F4F6')
                self.canvas_contours.axes.yaxis.label.set_color('#F3F4F6')
                self.canvas_contours.axes.title.set_color('#3B82F6')
                ...
                self.canvas_contours.axes.set_xlabel(xlabel, color='#F3F4F6')
                self.canvas_contours.axes.set_ylabel(ylabel, color='#F3F4F6')
                self.canvas_contours.axes.set_title(t_title, color='#3B82F6')
    ...
    def show_about_dialog(self):
        ...
        if self.language == "EN":
            texto_sobre = (
                ...
                "<p><b>App:</b> IBSimion (v2.0.1)</p>"
                "<p><b>Status:</b> Stable Beta Version</p>"
                "<p><b>Dev:</b> mosquitobr</p>"

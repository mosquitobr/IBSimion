...
# IBSimion v2.0.1.e4l - Manual de Operação Científica / Scientific Operations Manual
...
#### E. Ajuste de Tamanho de Fonte e Resolução (Acessibilidade)
O aplicativo possui seletores de **Tamanho da Janela** (Resolução) e **Tamanho da Fonte** no canto superior direito do menu principal.
1. **Tamanho da Fonte**: Escolha entre 9pt e 16pt. Todas as caixas, labels, tabelas e gráficos Matplotlib serão escalados dinamicamente em tempo real para legibilidade ideal.
2. **Tamanho da Janela**: Ajuste instantaneamente a janela para resoluções padrão (ex: 1024x768, 1280x720, 1360x860 (padrão), 1440x900, 1600x900, 1920x1080) para caber na sua tela física ou monitor virtual.
...
#### E. Font Sizing & Window Resolution (Accessibility)
The application provides **Window Size** (Resolution) and **Font Size** dropdown menus at the top right corner of the main menu bar.
1. **Font Sizing**: Choose from 9pt to 16pt. All UI panels, text blocks, configurations, and Matplotlib plots update dynamically in real-time.
2. **Window Resolution**: Resize the workspace to common desktop resolutions (e.g., 1024x768, 1280x720, 1360x860 (default), 1440x900, 1600x900, 1920x1080) for display comfort.

...
def calculate_twiss_from_tof(tof_path, plane='X'):
        ...
        for line in f:
            ...
            if len(parts) >= 7:
                try:
                    ...
                    if plane == 'Y':
                        pos_coords.append(float(parts[3]))
                        vel_coords.append(float(parts[4]))
                    else:
                        pos_coords.append(float(parts[1]))
                        vel_coords.append(float(parts[2]))
                    vz_coords.append(float(parts[6]))
                except ValueError:
                    pass
    
    num_particles = len(pos_coords)
    if num_particles == 0:
        return None
        
    xp = [v / vz if vz != 0 else 0.0 for v, vz in zip(vel_coords, vz_coords)]
    
    mean_x = sum(pos_coords) / num_particles
    mean_xp = sum(xp) / num_particles
    
    dx = [x - mean_x for x in pos_coords]
...
def run_test4():
    print("\n==========================================")
    print("RUNNING TEST 4: SLIT3D (High-Res STL Simulation)")
    print("==========================================")
    
    test_dir = os.path.join(BENT_DIR, "teste4")
    wsl_test_dir = f"{WSL_BENT_DIR}/teste4"
    ...
    # Compile native benchmark
    print("Compiling native slit3d...")
    build_res = run_cmd_wsl(f"cd {wsl_test_dir} && make clean && make")
    if build_res.returncode != 0:
        print("Error compiling slit3d:")
        print(build_res.stderr)
        return False, "Compilation error"
        
    # Run native benchmark
    print("Running native slit3d...")
    run_cmd_wsl(f"cd {wsl_test_dir} && ./slit3d")
    ...
    scenario_json = {
        ...
        "mirror": [True, False, True, False, False, False],
        "geometries": [
            {
                "name": "slit_assembly_plasma_1",
                "file_path": "../bent/teste4/slit_assembly_plasma_1.stl",
                "voltage": 0.0,
                "type": "Dirichlet",
                "translation": [-0.375303, -0.306421, 0.233167],
                "scale": 1.0
            },
            {
                "name": "slit_assembly_puller_2",
                "file_path": "../bent/teste4/slit_assembly_puller_2.stl",
                "voltage": -50000.0,
                "type": "Dirichlet",
                "translation": [-0.375303, -0.306421, 0.233167],
                "scale": 1.0
            ...
            {
                "name": "slit_assembly_gnd_3",
                "file_path": "../bent/teste4/slit_assembly_gnd_3.stl",
                "voltage": -40000.0,
                "type": "Dirichlet",
                "translation": [-0.375303, -0.306421, 0.233167],
                "scale": 1.0
    ...
    # Run wrapper simulation
    print("Running ibsimu_wrapper for test 4...")
    run_res = run_cmd_wsl(f"cd {WSL_BACKEND_DIR} && ./ibsimu_wrapper ../bent/teste4/config_scenario.json")
    if run_res.returncode != 0:
        print("Error running wrapper:")
        print(run_res.stderr)
        return False, "Wrapper run error"
        
    wrapper_metrics = calculate_twiss_from_tof(wrapper_tof, plane='Y')
    if not wrapper_metrics:
        return False, "Failed to parse wrapper metrics"

...
# Configurações de fontes maiores para plots do Matplotlib (High-DPI)
...
TRANSLATIONS = {
    "PT": {
        ...
        "subtab_elements": "2. Elementos e Eletrodos",
        ...
        "map_2d_style_box": "Estilo do Mapa de Campo 2D",
        ...
        "beam_headers": [
            ...
            "Raio/Tam. X (m)", "Tam. Y (m)", "Temp. Trans. Tt (eV)", "Temp. Para. Tp (eV)",
        ...
        "btn_rewind_pic": "⏪ Retroceder",
        "btn_forward_pic": "⏩ Avançar",
        "lbl_speed_pic": "Velocidade:",
        "chk_loop_pic": "Loop Ativo",
        "chk_fading_trail": "Rastro Suave",
        "lbl_trail_opacity": "Opacidade do Rastro:",
        "btn_export_snapshot": "Exportar Snapshot (PNG)",
        "btn_record_video": "Gravar Vídeo (MP4/GIF)",
        "group_clipping": "Fatiamento Geométrico (Clip Planes)",
        "chk_clip_x": "Corte X",
        "chk_clip_y": "Corte Y",
        "chk_clip_z": "Corte Z",
        "lbl_cw_step": "Amostragem:",
        "group_overlay": "Sobreposição Diagnóstica (Heatmap Overlay)",
        "chk_overlay_active": "Ativar Mapa",
        "overlay_potential": "Potencial Elétrico (V)",
        "overlay_rho": "Densidade de Carga (Rho)",
        "overlay_j": "Densidade de Corrente (J)",
    ...
    "EN": {
        ...
        "subtab_elements": "2. Elements & Electrodes",
        ...
        "map_2d_style_box": "2D Field Map Style",
        ...
        "beam_headers": [
            ...
            "Radius/Size X (m)", "Size Y (m)", "Trans. Temp. Tt (eV)", "Long. Temp. Tp (eV)",
            "Orig. Z (m)", "Orig. X (m)", "Orig. Y (m)", "Dir. X (ux)", "Dir. Z (uz)"
        ],
        ...
        "btn_rewind_pic": "⏪ Step Back",
        "btn_forward_pic": "⏩ Step Fwd",
        "lbl_speed_pic": "Speed:",
        "chk_loop_pic": "Active Loop",
        "chk_fading_trail": "Fading Trail",
        "lbl_trail_opacity": "Trail Opacity:",
        "btn_export_snapshot": "Export Snapshot (PNG)",
        "btn_record_video": "Record Video (MP4/GIF)",
        "group_clipping": "Geometric Clipping (Clip Planes)",
        "chk_clip_x": "Clip X",
        "chk_clip_y": "Clip Y",
        "chk_clip_z": "Clip Z",
        "lbl_cw_step": "Sampling:",
        "group_overlay": "Diagnostic Overlay (Heatmap Overlay)",
        "chk_overlay_active": "Enable Heatmap",
    ...
    def __init__(self, parent=None, language="PT"):
        ...
        if self.language == "PT":
            self.manual_chapters = {
                ...
                """,
                "Acessibilidade & Resolução": """
                    <h1 style='color: #3B82F6; border-bottom: 2px solid #3B82F6; padding-bottom: 5px;'>Acessibilidade, Resolução e Aprimoramentos Visuais</h1>
                    <p>O IBSimion permite o ajuste dinâmico do tamanho das fontes e da janela principal do aplicativo em tempo real para acomodar diferentes telas e resoluções High-DPI.</p>
                    <ul>
                        <li><b>Ajuste de Fonte (ComboBox de Tamanho):</b> Localizado no canto superior direito do menu principal. Permite selecionar tamanhos de fonte de 9pt a 16pt. Ao alterar a seleção, todas as janelas, tabelas, caixas de configuração e os textos de eixos dos gráficos de emitância e tempo de voo são escalados proporcionalmente em tempo real.</li>
                        <li><b>Ajuste da Janela (ComboBox de Resolução):</b> Localizado ao lado do ajuste de fonte no canto superior direito. Permite redimensionar instantaneamente a janela para as resoluções mais comuns (ex: 1024x768, 1280x720, 1360x860 (padrão), 1440x900, 1600x900, 1920x1080).</li>
                        <li><b>Modo Escuro / Modo Claro:</b> Alterna de forma síncrona o contraste visual de todos os elementos e também atualiza o fundo e contornos das malhas CAD no visualizador 3D (PyVista) e os temas dos gráficos estatísticos de simulação para manter alta visibilidade.</li>
                    </ul>
                """
            }
        else:
            self.manual_chapters = {
                ...
                """,
                "Accessibility & Resolution": """
                    <h1 style='color: #3B82F6; border-bottom: 2px solid #3B82F6; padding-bottom: 5px;'>Accessibility, Resolution & Sizing</h1>
                    <p>IBSimion permits dynamic scaling of both font sizes and window dimensions in real-time to accommodate High-DPI screens and portable monitors.</p>
                    <ul>
                        <li><b>Font Scaling (Size ComboBox):</b> Located at the top right of the menu bar. Allows choosing sizes from 9pt to 16pt. When updated, all interface layouts, tables, configuration panels, and Matplotlib plot labels adjust dynamically.</li>
                        <li><b>Window Sizing (Resolution ComboBox):</b> Sits next to the font selector. Resizes the main window instantly to target resolutions (e.g., 1024x768, 1280x720, 1360x860 (default), 1440x900, 1600x900, 1920x1080).</li>
                        <li><b>Dark / Light Themes:</b> Synchronously toggles contrast colors of application widgets, 3D electrodes boundary contours in PyVista, and simulation plotting backgrounds to ensure high contrast in any lighting environment.</li>
                    </ul>
                """
            }
...
class SimWorker(QThread):
    ...
    def write_config(self, filepath, params):
        ...
        if "beams" in p_copy and isinstance(p_copy["beams"], list):
            for beam in p_copy["beams"]:
                dist = beam.get("distribution", "Uniform")
                if dist in ["Retangular", "rectangular", "Rectangular"]:
                    beam["shape"] = "rectangular"
                    beam["tam_x"] = beam.get("tam_x", beam.get("size1", beam.get("radius", 0.0005)))
                    beam["tam_y"] = beam.get("tam_y", beam.get("size2", beam.get("tam_y", 0.0005)))
                    beam["size1"] = beam["tam_x"]
                    beam["size2"] = beam["tam_y"]
                else:
                    beam["shape"] = "cylindrical"
...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.combo_winsize = QComboBox()
        self.combo_winsize.addItems(["1024x768", "1280x720", "1360x860", "1440x900", "1600x900", "1920x1080"])
        self.combo_winsize.setCurrentText("1360x860")
        self.combo_winsize.currentIndexChanged.connect(lambda: self.change_window_size(self.combo_winsize.currentText()))
        self.combo_winsize.setStyleSheet("""
            QComboBox {
                background-color: #1F2937;
                border: 1px solid #3B82F6;
                border-radius: 4px;
        ...
        """)
        corner_layout.addWidget(self.combo_winsize)

        # Font Size Selector
        self.combo_fontsize = QComboBox()
        self.combo_fontsize.addItems(["9pt", "10pt", "11pt", "12pt", "14pt", "16pt"])
        self.combo_fontsize.setCurrentText("10pt")
        self.combo_fontsize.currentIndexChanged.connect(lambda: self.change_font_size(self.combo_fontsize.currentText()))
        self.combo_fontsize.setStyleSheet("""
            QComboBox {
                background-color: #1F2937;
                border: 1px solid #3B82F6;
                border-radius: 4px;
        ...
        """)
        corner_layout.addWidget(self.combo_fontsize)
    ...
    def create_config_tab(self):
        ...
        # Instantiate mesh widgets first (for signal connections safety)
        self.cb_domain_type = QComboBox()
        self.cb_domain_type.addItems(["3D Cartesiano Puro", "2D Cilíndrico (Simetria Axissimétrica)", "2D Cartesiano Estrito", "2D Cilíndrico Especial (2DCYL)"])
        self.cb_domain_type.currentIndexChanged.connect(self.on_domain_type_changed)
        
        self.txt_h = QLineEdit("0.001")
        from PySide6.QtGui import QDoubleValidator
        h_val = QDoubleValidator(0.0001, 0.1, 6, self.txt_h)
        h_val.setNotation(QDoubleValidator.StandardNotation)
        self.txt_h.setValidator(h_val)
        ...
        self.cb_beam_dist.addItems(["Uniform", "Gaussian", "Waterbag", "Retangular"])
        self.cb_beam_dist.currentIndexChanged.connect(self.on_beam_editor_changed)
        
        self.rb_energy = QRadioButton("Energia (eV)")
        self.rb_velocity = QRadioButton("Velocidade (m/s)")
        self.rb_energy.setChecked(True)
        
        self.bg_input_type = QButtonGroup(self)
        self.bg_input_type.addButton(self.rb_energy)
        self.bg_input_type.addButton(self.rb_velocity)
        
        self.rb_energy.toggled.connect(self.on_beam_input_type_toggled)
        self.rb_velocity.toggled.connect(self.on_beam_input_type_toggled)
        
        self.lbl_beam_val = QLabel("Valor da Energia (eV):")
        self.txt_beam_val = QLineEdit("1000.0")
        self.txt_beam_val.textChanged.connect(self.on_beam_editor_changed)
        
        self.beam_summary = QTextEdit()
        self.beam_summary.setReadOnly(True)

        # Left Panel (Regime setting & suggestions)
        ...
        self.table_beams.setColumnCount(17)
        ...
        # Distribuição
        self.lbl_beam_dist = QLabel("Distribuição:")
        beam_editor_layout.addWidget(self.lbl_beam_dist, 0, 0)
        beam_editor_layout.addWidget(self.cb_beam_dist, 0, 1)
        
        # Chaveamento Energia vs Velocidade
        self.lbl_input_type = QLabel("Tipo de Entrada de Feixe:")
        hbox_type = QHBoxLayout()
        hbox_type.addWidget(self.rb_energy)
        hbox_type.addWidget(self.rb_velocity)
        beam_editor_layout.addWidget(self.lbl_input_type, 1, 0)
        beam_editor_layout.addLayout(hbox_type, 1, 1)
        
        # Campo para o valor de Energia ou Velocidade
        beam_editor_layout.addWidget(self.lbl_beam_val, 2, 0)
        beam_editor_layout.addWidget(self.txt_beam_val, 2, 1)
        
        # QTextEdit para sumário (reduzido, para dar feedback visual)
        ...
        """)
        self.beam_summary.setMaximumHeight(80)
        beam_editor_layout.addWidget(self.beam_summary, 3, 0, 1, 2)
    ...
    def create_workstation_tab(self):
        ...
        # Left sidebar: Tabbed layout for ergonomics
        left_panel = QVBoxLayout()
        self.left_tabs = QTabWidget()
        
        # --- SUB-ABA 1: Domínio Global ---
        sub_tab_domain = QWidget()
        domain_layout = QVBoxLayout(sub_tab_domain)
        
        # Mesh settings & external field GroupBox (Decoupled from config tab)
        self.mesh_box = QGroupBox("Arquitetura de Malhas e Mapas Magnéticos")
        grid_mesh = QGridLayout(self.mesh_box)
        
        # 1. Tipo de Domínio Global
        self.lbl_domain_type = QLabel("Tipo de Domínio Global:")
        grid_mesh.addWidget(self.lbl_domain_type, 0, 0)
        grid_mesh.addWidget(self.cb_domain_type, 0, 1)
        
        # 2. Passo da Malha h
        self.lbl_h = QLabel("Passo da Malha h (m):")
        grid_mesh.addWidget(self.lbl_h, 1, 0)
        grid_mesh.addWidget(self.txt_h, 1, 1)
        
        # 3. Limite Z max
        self.lbl_zmax = QLabel("Tamanho Z max (m):")
        grid_mesh.addWidget(self.lbl_zmax, 2, 0)
        grid_mesh.addWidget(self.txt_zmax, 2, 1)
        
        # 4. Limite radial R max
        self.lbl_rmax = QLabel("Limite radial R max (m):")
        grid_mesh.addWidget(self.lbl_rmax, 3, 0)
        grid_mesh.addWidget(self.txt_rmax, 3, 1)
        
        # 5. Tamanho X max
        self.lbl_xmax = QLabel("Tamanho X max (m):")
        grid_mesh.addWidget(self.lbl_xmax, 4, 0)
        grid_mesh.addWidget(self.txt_xmax, 4, 1)
        
        # 6. Tamanho Y max
        self.lbl_ymax = QLabel("Tamanho Y max (m):")
        ...
        # --- SUB-ABA 2: Elementos e Eletrodos ---
        sub_tab_solids = QWidget()
        solids_main_layout = QVBoxLayout(sub_tab_solids)
        
        # Solids manager tree list & lateral properties fields
        ...
        solids_main_layout.addWidget(self.solids_box, 2)
        ...
        self.editor_tx.setRange(-10.0, 10.0)
        self.editor_tx.setDecimals(6)
        self.editor_tx.setSingleStep(0.001)
        self.editor_ty = QDoubleSpinBox()
        self.editor_ty.setRange(-10.0, 10.0)
        self.editor_ty.setDecimals(6)
        self.editor_ty.setSingleStep(0.001)
        self.editor_tz = QDoubleSpinBox()
        self.editor_tz.setRange(-10.0, 10.0)
        self.editor_tz.setDecimals(6)
        self.editor_tz.setSingleStep(0.001)
        ...
        # Modo de Sólido 3D
        self.lbl_editor_mapping = QLabel("Mapeamento Transversal 3D:")
        editor_layout.addWidget(self.lbl_editor_mapping, 7, 0)
        self.editor_mapping = QComboBox()
        self.editor_mapping.addItems(["Rotação em Z (rotz)", "Rotação em X (rotx)", "Extrusão Planar"])
        ...
        solids_main_layout.addWidget(self.group_editor, 3)
        
        # Add sub-tabs to left tab widget
        self.left_tabs.addTab(sub_tab_domain, "1. Domínio Global")
        self.left_tabs.addTab(sub_tab_solids, "2. Elementos e Eletrodos")
        ...
        # Fatiamento Geométrico GroupBox
        self.group_clipping = QGroupBox("Fatiamento Geométrico (Clip Planes)")
        grid_clipping = QGridLayout(self.group_clipping)
        
        self.chk_clip_x = QCheckBox("Corte X")
        self.chk_clip_x.stateChanged.connect(self.on_clipping_changed)
        self.slider_clip_x = QSlider(Qt.Horizontal)
        self.slider_clip_x.valueChanged.connect(self.on_clipping_changed)
        self.lbl_clip_x = QLabel("X: Desativado")
        grid_clipping.addWidget(self.chk_clip_x, 0, 0)
        grid_clipping.addWidget(self.slider_clip_x, 0, 1)
        grid_clipping.addWidget(self.lbl_clip_x, 0, 2)
        
        self.chk_clip_y = QCheckBox("Corte Y")
        self.chk_clip_y.stateChanged.connect(self.on_clipping_changed)
        self.slider_clip_y = QSlider(Qt.Horizontal)
        self.slider_clip_y.valueChanged.connect(self.on_clipping_changed)
        self.lbl_clip_y = QLabel("Y: Desativado")
        grid_clipping.addWidget(self.chk_clip_y, 1, 0)
        grid_clipping.addWidget(self.slider_clip_y, 1, 1)
        grid_clipping.addWidget(self.lbl_clip_y, 1, 2)
        
        self.chk_clip_z = QCheckBox("Corte Z")
        self.chk_clip_z.stateChanged.connect(self.on_clipping_changed)
        self.slider_clip_z = QSlider(Qt.Horizontal)
        self.slider_clip_z.valueChanged.connect(self.on_clipping_changed)
        self.lbl_clip_z = QLabel("Z: Desativado")
        grid_clipping.addWidget(self.chk_clip_z, 2, 0)
        grid_clipping.addWidget(self.slider_clip_z, 2, 1)
        grid_clipping.addWidget(self.lbl_clip_z, 2, 2)
        
        right_panel.addWidget(self.group_clipping)
        
        # Sobreposição Diagnóstica GroupBox
        self.group_overlay = QGroupBox("Sobreposição Diagnóstica (Heatmap Overlay)")
        lay_overlay = QHBoxLayout(self.group_overlay)
        
        self.chk_overlay_active = QCheckBox("Ativar Sobreposição")
        self.chk_overlay_active.stateChanged.connect(self.update_heatmap_overlay)
        lay_overlay.addWidget(self.chk_overlay_active)
        
        self.cb_overlay_field = QComboBox()
        self.cb_overlay_field.addItems(["Potencial Elétrico (V)", "Densidade de Carga (Rho)", "Densidade de Corrente (J)", "Campo Elétrico (E)"])
        self.cb_overlay_field.currentIndexChanged.connect(self.update_heatmap_overlay)
        lay_overlay.addWidget(self.cb_overlay_field)
        
        self.cb_overlay_scale = QComboBox()
        self.cb_overlay_scale.addItems(["Linear", "Logarítmica"])
        self.cb_overlay_scale.currentIndexChanged.connect(self.update_heatmap_overlay)
        lay_overlay.addWidget(self.cb_overlay_scale)
        
        right_panel.addWidget(self.group_overlay)
        
        # PIC Animation control box
        self.group_pic_anim = QGroupBox("Controle de Animação PIC")
        self.group_pic_anim.setEnabled(False)
        pic_v_layout = QVBoxLayout(self.group_pic_anim)
        
        # Row 1: Slider and Time Label
        pic_row1 = QHBoxLayout()
        self.slider_pic = QSlider(Qt.Horizontal)
        self.slider_pic.valueChanged.connect(self.on_pic_slider_changed)
        pic_row1.addWidget(self.slider_pic)
        
        self.lbl_pic_time = QLabel("Tempo: 0.0s")
        pic_row1.addWidget(self.lbl_pic_time)
        pic_v_layout.addLayout(pic_row1)
        
        # Row 2: Playback Controls (Step Back, Play/Pause, Step Forward)
        pic_row2 = QHBoxLayout()
        self.btn_rewind_pic = QPushButton("⏪")
        self.btn_rewind_pic.clicked.connect(self.step_backward_pic_animation)
        pic_row2.addWidget(self.btn_rewind_pic)
        
        self.btn_play_pic = QPushButton("Animar (Play)")
        self.btn_play_pic.clicked.connect(self.toggle_pic_animation)
        pic_row2.addWidget(self.btn_play_pic)
        
        self.btn_forward_pic = QPushButton("⏩")
        self.btn_forward_pic.clicked.connect(self.step_forward_pic_animation)
        pic_row2.addWidget(self.btn_forward_pic)
        
        pic_v_layout.addLayout(pic_row2)
        
        # Row 3: Animation Settings (Speed, Loop, Fading Trail)
        pic_row3 = QHBoxLayout()
        self.lbl_speed_pic = QLabel("Velocidade:")
        pic_row3.addWidget(self.lbl_speed_pic)
        self.cb_speed_pic = QComboBox()
        self.cb_speed_pic.addItems(["0.5x", "1.0x", "2.0x", "5.0x"])
        self.cb_speed_pic.setCurrentIndex(1)  # 1.0x
        self.cb_speed_pic.currentIndexChanged.connect(self.on_speed_changed)
        pic_row3.addWidget(self.cb_speed_pic)
        
        self.chk_loop_pic = QCheckBox("Loop Ativo")
        self.chk_loop_pic.setChecked(True)
        pic_row3.addWidget(self.chk_loop_pic)
        
        self.chk_fading_trail = QCheckBox("Rastro Suave")
        self.chk_fading_trail.setChecked(False)
        self.chk_fading_trail.stateChanged.connect(self.on_fading_trail_changed)
        pic_row3.addWidget(self.chk_fading_trail)
        
        self.lbl_trail_opacity = QLabel("Opacidade Rastro:")
        pic_row3.addWidget(self.lbl_trail_opacity)
        self.slider_trail_opacity = QSlider(Qt.Horizontal)
        self.slider_trail_opacity.setRange(0, 100)
        self.slider_trail_opacity.setValue(85)
        self.slider_trail_opacity.setFixedWidth(100)
        self.slider_trail_opacity.valueChanged.connect(self.on_trail_opacity_changed)
        pic_row3.addWidget(self.slider_trail_opacity)
        
        pic_v_layout.addLayout(pic_row3)
        
        # Row 4: Media Export (Snapshot PNG, Record Video/GIF)
        pic_row4 = QHBoxLayout()
        self.btn_export_snapshot = QPushButton("Exportar Snapshot (PNG)")
        self.btn_export_snapshot.clicked.connect(self.export_snapshot_png)
        pic_row4.addWidget(self.btn_export_snapshot)
        
        self.btn_record_video = QPushButton("Gravar Vídeo (MP4/GIF)")
        self.btn_record_video.clicked.connect(self.record_video_media)
        pic_row4.addWidget(self.btn_record_video)
        
        pic_v_layout.addLayout(pic_row4)
    ...
    def create_diagnostics_tab(self):
        ...
        self.cb_plane_orient.currentIndexChanged.connect(self.update_heatmap_overlay)
        ...
        # Estilo do Mapa de Campo 2D
        self.map_2d_style_box = QGroupBox("Estilo do Mapa de Campo 2D")
        grid_map_2d = QGridLayout(self.map_2d_style_box)
        
        self.lbl_map_2d = QLabel("Tipo de Mapa 2D:")
        grid_map_2d.addWidget(self.lbl_map_2d, 0, 0)
    ...
    def browse_geom_file(self):
        ...
        if not selected_rows:
            return
        domain_type = self.cb_domain_type.currentText()
        is_3d = "3D" in domain_type
        filter_str = "CAD Files (*.stl *.dxf);;STL Files (*.stl);;DXF Files (*.dxf)" if is_3d else "DXF Files (*.dxf)"
        fn, _ = QFileDialog.getOpenFileName(self, "Selecionar Arquivo CAD/Sólido", "", filter_str)
        if fn:
    ...
    def update_solids_editor_mapping_visibility(self):
        ...
        is_3d = self.cb_domain_type.currentIndex() == 0
        
        if is_dxf and is_3d:
            self.lbl_editor_mapping.show()
            self.editor_mapping.show()
            self.editor_mapping.setEnabled(True)
        else:
            self.lbl_editor_mapping.hide()
            self.editor_mapping.hide()
        ...
        try:
            ...
            if self.cb_regime.currentText() == "PIC":
                if hasattr(self, "beams") and self.beams:
                    ions = [b for b in self.beams if b.get("massa", 136.2) >= 0.1]
                    target_beams = ions if ions else self.beams
                    velocities = [1.389e4 * np.sqrt(b.get("energy", 1000.0) / b.get("massa", 136.2)) for b in target_beams if b.get("massa", 136.2) > 0]
                    if velocities:
    ...
    def reload_geometries_table(self):
        ...
        for idx, geom in enumerate(self.geometries):
            ...
            if geom["file_path"].lower().endswith(".stl"):
                mapping_str = "N/A (STL)"
            else:
            ...
            self.table_geoms.setItem(idx, 5, QTableWidgetItem(f"{geom['translation'][2]:.6f}"))
    ...
    def add_geometry_row(self):
        ...
        # Copia automática do arquivo CAD para a pasta data/ do projeto se já não estiver lá
        try:
            script_dir = os.path.dirname(os.path.abspath(__file__))
            repo_root = os.path.dirname(script_dir)
            ...
            target_fn = os.path.join(data_dir, filename)
            
            if os.path.abspath(fn) != os.path.abspath(target_fn):
                shutil.copy2(fn, target_fn)
            
            fn = target_fn
        except Exception as e:
            print(f"Error copying CAD file to data/ directory: {e}")
    ...
    def on_geometry_selected(self):
        ...
        # Trava lógica contra Neumann bug (força Dirichlet e desabilita)
        self.editor_btype.setCurrentText("Dirichlet")
        self.editor_btype.setEnabled(False)
        ...
        elif mapping_val == "unity" or mapping_val == "extrude":
            self.editor_mapping.setCurrentIndex(2)
        else:
        ...
        if is_dxf:
            self.editor_layer.setEnabled(True)
            self.editor_mapping.setEnabled(True)
            self.lbl_editor_mapping.setEnabled(True)
            ...
            self.editor_layer.setEnabled(False)
            self.editor_mapping.setEnabled(False)
            self.lbl_editor_mapping.setEnabled(False)
    ...
    def apply_sidebar_edits(self):
        ...
        if is_dxf:
            idx = self.editor_mapping.currentIndex()
            if idx == 0:
                geom["mapping"] = "rotz"
            elif idx == 1:
                geom["mapping"] = "rotx"
    ...
    def on_beam_table_edited(self, item):
        ...
        try:
            if col == 0:
                ...
                beam["radius"] = float(val)
            elif col == 9:
                beam["tam_y"] = float(val)
            elif col == 10:
                beam["Tt"] = float(val)
            elif col == 11:
                beam["Tp"] = float(val)
    ...
    def on_beam_selected(self):
        ...
        # Block signals to prevent triggering on_beam_editor_changed during load
        self.cb_beam_dist.blockSignals(True)
        self.rb_energy.blockSignals(True)
        self.rb_velocity.blockSignals(True)
        self.txt_beam_val.blockSignals(True)
        
        # 1. Distribuição
        dist = beam.get("distribution", "Uniform")
        self.cb_beam_dist.setCurrentText(dist)
        
        # 2. Tipo de Entrada (Energia vs Velocidade)
        input_type = beam.get("beam_input_type", "energia")
        if input_type == "velocidade":
            self.rb_velocity.setChecked(True)
            self.lbl_beam_val.setText("Valor da Velocidade (m/s):")
            self.txt_beam_val.setText(str(beam.get("velocity", 0.0)))
        else:
            self.rb_energy.setChecked(True)
            self.lbl_beam_val.setText("Valor da Energia (eV):")
            self.txt_beam_val.setText(str(beam.get("energy", 1000.0)))
            
        self.cb_beam_dist.blockSignals(False)
        self.rb_energy.blockSignals(False)
        self.rb_velocity.blockSignals(False)
        self.txt_beam_val.blockSignals(False)
        
        self.update_beam_summary()

    def on_beam_input_type_toggled(self):
        row = self.table_beams.currentRow()
        if row < 0 or row >= len(self.beams):
            return
        beam = self.beams[row]
        ...
        if self.rb_velocity.isChecked():
            self.lbl_beam_val.setText("Valor da Velocidade (m/s):")
            # Converter energia para velocidade se velocity não existir
            mass = beam.get("massa", 1.0)
            energy = beam.get("energy", 1000.0)
            import math
            v = 1.3884e4 * math.sqrt(energy / (mass + 1e-30))
            beam["velocity"] = v
            beam["beam_input_type"] = "velocidade"
            self.txt_beam_val.setText(f"{v:.4e}")
        else:
            self.lbl_beam_val.setText("Valor da Energia (eV):")
            # Converter velocidade para energia
            velocity = beam.get("velocity", 0.0)
            mass = beam.get("massa", 1.0)
            energy = mass * ((velocity / 1.3884e4) ** 2)
            beam["energy"] = energy
            beam["beam_input_type"] = "energia"
            self.txt_beam_val.setText(f"{energy:.1f}")
            
        self.txt_beam_val.blockSignals(False)
        self.reload_beams_table()
        self.update_beam_summary()

    def on_beam_editor_changed(self):
        row = self.table_beams.currentRow()
        if row < 0 or row >= len(self.beams):
            return
        beam = self.beams[row]
        ...
        # Save distribution
        beam["distribution"] = self.cb_beam_dist.currentText()
        
        # Save input value
        try:
            val = float(self.txt_beam_val.text() or 0.0)
        except ValueError:
            val = 0.0
        ...
        if self.rb_velocity.isChecked():
            beam["velocity"] = val
            beam["beam_input_type"] = "velocidade"
            # Recalcular energia para exibição na tabela
            mass = beam.get("massa", 1.0)
            beam["energy"] = mass * ((val / 1.3884e4) ** 2)
        else:
            beam["energy"] = val
            beam["beam_input_type"] = "energia"
        ...
        self.table_beams.blockSignals(True)
        self.reload_beams_table()
        self.table_beams.setCurrentCell(row, self.table_beams.currentColumn())
        self.table_beams.blockSignals(False)
        self.update_beam_summary()
    ...
    def update_beam_summary(self):
        ...
        beam = self.beams[row]
        dist = beam.get("distribution", "Uniform")
        is_rect = dist in ["Retangular", "rectangular", "Rectangular"]
        
        if self.language == "PT":
            size_html = f"""
            <tr><td style="padding: 3px 0; color: #9CA3AF;">Largura (Tam. X):</td><td style="text-align: right; font-weight: bold;">{beam.get('radius', 0.0005):.5f} m</td></tr>
            <tr><td style="padding: 3px 0; color: #9CA3AF;">Altura (Tam. Y):</td><td style="text-align: right; font-weight: bold;">{beam.get('tam_y', 0.0005):.5f} m</td></tr>
            """ if is_rect else f"""
            <tr><td style="padding: 3px 0; color: #9CA3AF;">Raio:</td><td style="text-align: right; font-weight: bold;">{beam.get('radius', 0.0005):.5f} m</td></tr>
            """
            ...
            <table style="width: 100%; border-collapse: collapse; color: #F3F4F6;">
                ...
                {size_html}
                <tr><td style="padding: 3px 0; color: #9CA3AF;">Temp. Trans. Tt:</td><td style="text-align: right; font-weight: bold;">{beam.get('Tt', 0.0):.4f} eV</td></tr>
                <tr><td style="padding: 3px 0; color: #9CA3AF;">Temp. Para. Tp:</td><td style="text-align: right; font-weight: bold;">{beam.get('Tp', 0.02):.4f} eV</td></tr>
            ...
            size_html = f"""
            <tr><td style="padding: 3px 0; color: #9CA3AF;">Width (Tam. X):</td><td style="text-align: right; font-weight: bold;">{beam.get('radius', 0.0005):.5f} m</td></tr>
            <tr><td style="padding: 3px 0; color: #9CA3AF;">Height (Tam. Y):</td><td style="text-align: right; font-weight: bold;">{beam.get('tam_y', 0.0005):.5f} m</td></tr>
            """ if is_rect else f"""
            <tr><td style="padding: 3px 0; color: #9CA3AF;">Radius:</td><td style="text-align: right; font-weight: bold;">{beam.get('radius', 0.0005):.5f} m</td></tr>
            """
    ...
    def run_simple_simulation(self):
        ...
        try:
            ...
            if total_nodes > 30_000_000:
                reply = QMessageBox.warning(
                    self, 
                    "Malha Densa" if self.language == "PT" else "Dense Mesh",
                    f"A resolução escolhida (h = {h} m) gerará aproximadamente {total_nodes:,} nós de malha.\n"
                    "Isso pode exigir muita memória RAM e tempo de CPU.\nDeseja continuar mesmo assim?"
                    if self.language == "PT" else
                    f"The chosen resolution (h = {h} m) will generate approximately {total_nodes:,} mesh nodes.\n"
                    "This may require a significant amount of RAM and CPU time.\nDo you want to continue anyway?",
    ...
    def reload_visualizer_scene(self):
        ...
        if regime == "CW":
            ...
            self.btn_rewind_pic.setEnabled(False)
            self.btn_play_pic.setEnabled(False)
            self.btn_forward_pic.setEnabled(False)
            self.cb_speed_pic.setEnabled(False)
            self.chk_loop_pic.setEnabled(False)
            self.chk_fading_trail.setEnabled(False)
            self.lbl_trail_opacity.setEnabled(False)
            self.slider_trail_opacity.setEnabled(False)
            self.btn_record_video.setEnabled(False)
            self.btn_export_snapshot.setEnabled(True)
            
            if self.animation_timer.isActive():
                self.animation_timer.stop()
            self.lbl_cw_step.setEnabled(True)
            self.spin_cw_step.setEnabled(True)
            ...
            sample_step = self.spin_cw_step.value()
            if os.path.exists(traj_path):
                self.pyvista_widget.load_trajectories(traj_path, color_by, sample_step=sample_step)
        else:
            self.group_pic_anim.setEnabled(True)
            self.slider_pic.setEnabled(True)
            self.btn_rewind_pic.setEnabled(True)
            self.btn_play_pic.setEnabled(True)
            self.btn_forward_pic.setEnabled(True)
            self.cb_speed_pic.setEnabled(True)
            self.chk_loop_pic.setEnabled(True)
            self.chk_fading_trail.setEnabled(True)
            self.lbl_trail_opacity.setEnabled(True)
            self.slider_trail_opacity.setEnabled(True)
            ...
            # Load trajectories in PIC mode too if trajectories.txt is present
            traj_path = os.path.join(self.backend_dir, "trajectories.txt")
            sample_step = self.spin_cw_step.value()
            if os.path.exists(traj_path):
                self.pyvista_widget.load_trajectories(traj_path, color_by='mass', sample_step=sample_step)
            ...
            try:
                ...
                if files:
                    ...
                    if self.pic_snapshots:
                        self.pyvista_widget.load_pic_snapshot(self.pic_snapshots[0], current_time=self.pic_times[0])
                        self.lbl_pic_time.setText(f"Tempo: {self.pic_times[0]:.2e} s")
            except Exception as e:
                print("Erro ao carregar snapshots PIC:", e)
        
        # Update geometric clipping
        self.update_clipping_slider_ranges()
        
        # Update heatmap overlay
        self.update_heatmap_overlay()
        
        # Enquadramento automático com foco inteligente nos atores ativos
        self.pyvista_widget.reset_camera_to_actors()
    ...
    def toggle_pic_animation(self):
        if self.animation_timer.isActive():
            self.animation_timer.stop()
            self.btn_play_pic.setText("Animar (Play)" if self.language == "PT" else "Play")
        else:
            if self.pic_snapshots:
                self.animation_timer.start()
                self.btn_play_pic.setText("Pausar" if self.language == "PT" else "Pause")
            else:
                QMessageBox.warning(self, "Simulação PIC Requerida", "Nenhum frame PIC encontrado. Execute primeiro.")

    def step_backward_pic_animation(self):
        if not self.pic_snapshots:
            return
        curr = self.slider_pic.value()
        prev_val = (curr - 1) % len(self.pic_snapshots)
        self.slider_pic.setValue(prev_val)

    def step_forward_pic_animation(self):
        if not self.pic_snapshots:
            return
        curr = self.slider_pic.value()
    ...
    def on_speed_changed(self):
        speed_str = self.cb_speed_pic.currentText().replace("x", "")
        try:
            factor = float(speed_str)
        except ValueError:
            factor = 1.0
        base_interval = 200  # ms
        self.animation_timer.setInterval(int(base_interval / factor))

    def on_fading_trail_changed(self, state):
        is_checked = (state == 2 or (hasattr(Qt, "CheckState") and state == Qt.CheckState.Checked) or getattr(Qt, "Checked", None) == state)
        self.pyvista_widget.fading_trail = is_checked
        # Force refresh current frame
        self.on_pic_slider_changed(self.slider_pic.value())

    def on_trail_opacity_changed(self, value):
        if hasattr(self, 'pyvista_widget'):
            self.pyvista_widget.trail_opacity = value / 100.0
            # Force refresh current frame
            self.on_pic_slider_changed(self.slider_pic.value())
    ...
    def advance_pic_animation(self):
        if not self.pic_snapshots:
            ...
            return
        curr = self.slider_pic.value()
        if curr >= len(self.pic_snapshots) - 1:
            if self.chk_loop_pic.isChecked():
                next_val = 0
            else:
                self.animation_timer.stop()
                self.btn_play_pic.setText("Animar (Play)" if self.language == "PT" else "Play")
    ...
    def on_pic_slider_changed(self, val):
        if 0 <= val < len(self.pic_snapshots):
            ...
            self.pyvista_widget.load_pic_snapshot(
                cached_data if cached_data is not None else self.pic_snapshots[val],
                current_time=self.pic_times[val]
            )
            self.lbl_pic_time.setText(f"Tempo: {self.pic_times[val]:.2e} s")

    def export_snapshot_png(self):
        file_path, _ = QFileDialog.getSaveFileName(
            self,
            "Exportar Snapshot PNG",
            "",
            "Imagens PNG (*.png)"
        )
        if file_path:
            success = self.pyvista_widget.export_screenshot(file_path)
            if success:
                self.lbl_status.setText(f"Snapshot exportado com sucesso: {os.path.basename(file_path)}")
            else:
                QMessageBox.critical(self, "Erro", "Falha ao gravar arquivo de imagem PNG.")

    def record_video_media(self):
        if not self.pic_snapshots:
            QMessageBox.warning(self, "Sem Frames", "Nenhum frame PIC disponível para gravar.")
            return
            
        file_path, selected_filter = QFileDialog.getSaveFileName(
            self,
            "Gravar Vídeo Animado",
            "",
            "Vídeo MP4 (*.mp4);;GIF Animado (*.gif)"
        )
        if not file_path:
            return
            
        # Garante a extensão correta com base no filtro selecionado se o usuário não digitou
        if not file_path.lower().endswith((".mp4", ".gif")):
            if "gif" in selected_filter.lower():
                file_path += ".gif"
            else:
                file_path += ".mp4"
        ...
        progress = QProgressDialog("Compilando frames sequenciais no buffer PyVista...", "Cancelar", 0, len(self.pic_snapshots), self)
        ...
        try:
            ...
            if is_gif:
                ...
                plotter.open_movie(file_path)
                
            for idx in range(len(self.pic_snapshots)):
                if progress.wasCanceled():
                    break
                
                progress.setValue(idx)
                self.slider_pic.setValue(idx)
            ...
            progress.setValue(len(self.pic_snapshots))
            
            if not progress.wasCanceled():
                QMessageBox.information(self, "Sucesso", f"Vídeo gravado em:\n{file_path}")
        except Exception as e:
            QMessageBox.critical(self, "Erro de Vídeo", f"Falha na compilação do vídeo: {e}")
        finally:
            if was_running:
                self.animation_timer.start()

    def update_clipping_slider_ranges(self):
        bounds = self.pyvista_widget.get_geometry_bounds()
        self.clip_bounds = bounds
        
        self.slider_clip_x.setRange(0, 1000)
        self.slider_clip_y.setRange(0, 1000)
        self.slider_clip_z.setRange(0, 1000)
        
        self.slider_clip_x.setValue(1000)
        self.slider_clip_y.setValue(1000)
        self.slider_clip_z.setValue(1000)
        
        self.chk_clip_x.setChecked(False)
        self.chk_clip_y.setChecked(False)
        self.chk_clip_z.setChecked(False)
        
        self.lbl_clip_x.setText("X: Desativado")
        self.lbl_clip_y.setText("Y: Desativado")
        self.lbl_clip_z.setText("Z: Desativado")

    def on_clipping_changed(self):
        if not hasattr(self, 'clip_bounds'):
            return
            
        b = self.clip_bounds
        
        clip_x = None
        if self.chk_clip_x.isChecked():
            val = self.slider_clip_x.value() / 1000.0
            clip_x = b[0] + val * (b[1] - b[0])
            self.lbl_clip_x.setText(f"X: {clip_x:.4f} m")
        else:
            self.lbl_clip_x.setText("X: Desativado")
            
        clip_y = None
        if self.chk_clip_y.isChecked():
            val = self.slider_clip_y.value() / 1000.0
            clip_y = b[2] + val * (b[3] - b[2])
            self.lbl_clip_y.setText(f"Y: {clip_y:.4f} m")
        else:
            self.lbl_clip_y.setText("Y: Desativado")
            
        clip_z = None
        if self.chk_clip_z.isChecked():
            val = self.slider_clip_z.value() / 1000.0
            clip_z = b[4] + val * (b[5] - b[4])
            self.lbl_clip_z.setText(f"Z: {clip_z:.4f} m")
        else:
            self.lbl_clip_z.setText("Z: Desativado")
            
        self.pyvista_widget.apply_reactive_clipping(clip_x, clip_y, clip_z)

    def update_heatmap_overlay(self):
        if not hasattr(self, 'pyvista_widget'):
            return
            
        # Clean previous actor and its scalar bar securely
        try:
            actors_keys = list(self.pyvista_widget.plotter.actors.keys())
            if "heatmap_overlay" in actors_keys:
                self.pyvista_widget.plotter.remove_actor("heatmap_overlay")
                
            last_title = getattr(self.pyvista_widget, "last_heatmap_title", None)
            if last_title:
                try:
                    self.pyvista_widget.plotter.remove_scalar_bar(title=last_title)
                except Exception:
                    pass
                self.pyvista_widget.last_heatmap_title = None
        except Exception as e:
            print(f"Error cleaning heatmap actors in update_heatmap_overlay: {e}")
            
        if not self.chk_overlay_active.isChecked():
        ...
        field_idx = self.cb_overlay_field.currentIndex()
        log_scale = (self.cb_overlay_scale.currentIndex() == 1)
        ...
        if field_idx == 0:  # Potential
            if os.path.exists(pot_field_path):
                try:
                    zs, xs, matrix = self.load_grid_slice(pot_field_path, 3, plane_orient=orient, coord_val=coord_val)
                except Exception as e:
                    print("Error loading potential overlay:", e)
        elif field_idx == 1:  # Rho (Charge density)
            if os.path.exists(rho_field_path):
                try:
                    zs, xs, matrix = self.load_grid_slice(rho_field_path, 3, plane_orient=orient, coord_val=coord_val)
                except Exception as e:
                    print("Error loading charge density overlay:", e)
            if matrix is None:
                zs, xs, matrix = self.calculate_density_map_from_trajectories(orient, coord_val, map_type=1)
        elif field_idx == 2:  # J (Current density)
            j_field_path = os.path.join(self.backend_dir, "current_density.dat")
            if os.path.exists(j_field_path):
                try:
                    # current_density.dat: X, Y, Z, Jx, Jy, Jz
                    zs, xs, Jx_mat = self.load_grid_slice(j_field_path, 3, plane_orient=orient, coord_val=coord_val)
                    _, _, Jy_mat = self.load_grid_slice(j_field_path, 4, plane_orient=orient, coord_val=coord_val)
                    _, _, Jz_mat = self.load_grid_slice(j_field_path, 5, plane_orient=orient, coord_val=coord_val)
                    if Jx_mat is not None and Jy_mat is not None and Jz_mat is not None:
                        matrix = np.sqrt(Jx_mat**2 + Jy_mat**2 + Jz_mat**2)
                except Exception as e:
                    print("Error loading current density overlay from current_density.dat:", e)
            if matrix is None:
                zs, xs, matrix = self.calculate_density_map_from_trajectories(orient, coord_val, map_type=2)
        elif field_idx == 3:  # Electric Field (E) magnitude
            if os.path.exists(pot_field_path):
                try:
                    # potential_field.dat: X, Y, Z, V, Ex, Ey, Ez
                    zs, xs, Ex_mat = self.load_grid_slice(pot_field_path, 4, plane_orient=orient, coord_val=coord_val)
                    _, _, Ey_mat = self.load_grid_slice(pot_field_path, 5, plane_orient=orient, coord_val=coord_val)
                    _, _, Ez_mat = self.load_grid_slice(pot_field_path, 6, plane_orient=orient, coord_val=coord_val)
                    if Ex_mat is not None and Ey_mat is not None and Ez_mat is not None:
                        matrix = np.sqrt(Ex_mat**2 + Ey_mat**2 + Ez_mat**2)
                except Exception as e:
                    print("Error loading electric field overlay:", e)
            
        if zs is not None and xs is not None and matrix is not None:
            self.pyvista_widget.render_heatmap_overlay(
                zs, xs, matrix,
                plane_orient=orient,
                coord_val=coord_val,
                log_scale=log_scale,
                title=self.cb_overlay_field.currentText()
            )

    def calculate_density_map_from_trajectories(self, orient, coord_val, map_type):
        traj_path = os.path.join(self.backend_dir, "trajectories.txt")
        ...
        try:
            ...
            for traj in trajectories:
                ...
                for k in range(len(pts) - 1):
                    ...
                    if len(pt1) == 4:
                        ...
                        t1, x1, y1, z1 = 0.0, pt1[0], pt1[1], pt1[2]
                        t2, x2, y2, z2 = 0.0, pt2[0], pt2[1], pt2[2]
                        
                    dt = t2 - t1
                    if dt <= 0:
                        # Estimate velocity: v = 1.3884e4 * sqrt(E / mass)
                        energy_val = 1000.0
                        mass_val = traj.get("mass", 136.2)
                        try:
                            for b_cfg in params.get("beams", []):
                                if abs(b_cfg.get("massa", 0.0) - mass_val) < 0.1:
                                    energy_val = b_cfg.get("energy", 1000.0)
                                    break
                        except Exception:
                            pass
                        
                        vel = 1.3884e4 * np.sqrt(max(0.1, energy_val) / max(1.0, mass_val))
            ...
            if len(coord_h) > 0:
                ...
                hs_centers = (h_edges[:-1] + h_edges[1:]) / 2.0
                vs_centers = (v_edges[:-1] + v_edges[1:]) / 2.0
                return hs_centers, vs_centers, matrix
        except Exception as e:
        ...
        if map_type == 0:
            if os.path.exists(pot_field_path):
                try:
                    ...
                    print("Error slicing potential file:", e)
            if os.path.exists(rho_field_path):
                try:
                    _, _, rho_matrix = self.load_grid_slice(rho_field_path, 3, plane_orient=orient, coord_val=coord_val)
                except Exception as e:
                    print("Error slicing charge density file:", e)
        elif map_type == 1 and os.path.exists(rho_field_path):
            try:
                zs, xs, rho_matrix = self.load_grid_slice(rho_field_path, 3, plane_orient=orient, coord_val=coord_val)
                if os.path.exists(pot_field_path):
                    _, _, V_matrix = self.load_grid_slice(pot_field_path, 3, plane_orient=orient, coord_val=coord_val)
            except Exception as e:
                print("Error slicing charge density file for Rho map:", e)
        elif map_type == 2:
            j_field_path = os.path.join(self.backend_dir, "current_density.dat")
            if os.path.exists(j_field_path):
                try:
                    ...
                    _, _, Jz_mat = self.load_grid_slice(j_field_path, 5, plane_orient=orient, coord_val=coord_val)
                    if Jx_mat is not None and Jy_mat is not None and Jz_mat is not None:
                        rho_matrix = np.sqrt(Jx_mat**2 + Jy_mat**2 + Jz_mat**2)
                    if os.path.exists(pot_field_path):
                        _, _, V_matrix = self.load_grid_slice(pot_field_path, 3, plane_orient=orient, coord_val=coord_val)
                except Exception as e:
                    print("Error slicing current density file for J map:", e)
                
        if rho_matrix is None and map_type > 0:
    ...
    def toggle_theme(self):
        ...
        if self.theme_mode == "dark":
            ...
            self.combo_winsize.setStyleSheet("QComboBox { background-color: #1F2937; border: 1px solid #3B82F6; border-radius: 4px; color: #F3F4F6; padding: 4px 8px; font-weight: bold; }")
            self.combo_fontsize.setStyleSheet("QComboBox { background-color: #1F2937; border: 1px solid #3B82F6; border-radius: 4px; color: #F3F4F6; padding: 4px 8px; font-weight: bold; }")
        else:
            ...
            self.combo_winsize.setStyleSheet("QComboBox { background-color: #FFFFFF; border: 1px solid #2563EB; border-radius: 4px; color: #111827; padding: 4px 8px; font-weight: bold; }")
            self.combo_fontsize.setStyleSheet("QComboBox { background-color: #FFFFFF; border: 1px solid #2563EB; border-radius: 4px; color: #111827; padding: 4px 8px; font-weight: bold; }")
    ...
    def change_font_size(self, size_text):
        try:
            size = int(size_text.replace("pt", "").strip())
            from PySide6.QtGui import QFont
            font = QApplication.font()
            font.setPointSize(size)
            QApplication.setFont(font)
            # Update Matplotlib font sizes proportionally
            matplotlib.rcParams['font.size'] = size + 1
            matplotlib.rcParams['axes.labelsize'] = size + 1
    ...
    def change_window_size(self, size_text):
        try:
    ...
    def retranslate_ui(self):
        ...
        # Sub-Tabs of Workstation
        if hasattr(self, "left_tabs"):
            self.left_tabs.setTabText(0, self.tr("subtab_global_domain"))
            self.left_tabs.setTabText(1, self.tr("subtab_elements"))
        ...
        self.btn_rewind_pic.setText(self.tr("btn_rewind_pic"))
        self.btn_forward_pic.setText(self.tr("btn_forward_pic"))
        self.lbl_speed_pic.setText(self.tr("lbl_speed_pic"))
        self.chk_loop_pic.setText(self.tr("chk_loop_pic"))
        self.chk_fading_trail.setText(self.tr("chk_fading_trail"))
        self.lbl_trail_opacity.setText(self.tr("lbl_trail_opacity"))
        self.btn_export_snapshot.setText(self.tr("btn_export_snapshot"))
        self.btn_record_video.setText(self.tr("btn_record_video"))
        
        self.group_clipping.setTitle(self.tr("group_clipping"))
        self.chk_clip_x.setText(self.tr("chk_clip_x"))
        self.chk_clip_y.setText(self.tr("chk_clip_y"))
        self.chk_clip_z.setText(self.tr("chk_clip_z"))
        
        self.lbl_cw_step.setText(self.tr("lbl_cw_step"))
        
        self.group_overlay.setTitle(self.tr("group_overlay"))
        self.chk_overlay_active.setText(self.tr("chk_overlay_active"))
        ...
        self.map_2d_style_box.setTitle(self.tr("map_2d_style_box"))
        ...
        self.cb_overlay_field.clear()
        self.cb_overlay_field.addItems([self.tr("overlay_potential"), self.tr("overlay_rho"), self.tr("overlay_j"), self.tr("overlay_efield")])
        self.cb_overlay_field.setCurrentIndex(idx_overlay if idx_overlay >= 0 else 0)
        self.cb_overlay_field.blockSignals(False)
    ...
    def show_about_dialog(self):
        ...
        if self.language == "EN":
            texto_sobre = (
                ...
                "<p><b>App:</b> IBSimion (v2.0.1.e4l)</p>"
    ...
    def apply_clipping_plane_diagnostics(self):
        ...
        try:
            ...
            # Sincroniza a re-renderização do heatmap com os limites do novo plano
            self.update_heatmap_overlay()
        except ValueError:
            QMessageBox.warning(self, "Valor Inválido" if self.language == "PT" else "Invalid Value",
    ...
    def calculate_pic_suggestions(self):
        ...
        try:
            ...
            # 1. Try to get from simulation cache (actual output data)
            tof_data = self.simulation_cache.get("tof")
            if tof_data is not None:
                if tof_data.ndim == 1:
                    tof_data = np.expand_dims(tof_data, axis=0)
                if tof_data.size > 0 and len(tof_data) > 0:
                    times = tof_data[:, 0]
                    masses = tof_data[:, 1] if tof_data.shape[1] > 1 else np.ones_like(times) * 136.2
                    vx = tof_data[:, 2]
                    vy = tof_data[:, 4]
                    vz = tof_data[:, 6]
                    speeds = np.sqrt(vx**2 + vy**2 + vz**2)
                    
                    # Filter out electrons (mass < 1e-28 kg) if ions are present
                    ions_mask = masses >= 1e-28
                    if np.any(ions_mask):
                        times = times[ions_mask]
                        speeds = speeds[ions_mask]
                    
                    if len(speeds) > 0:
                        v_max = np.max(speeds)
                        avg_tof = np.mean(times)
            
            # 2. Estimate from active beams if simulation cache is not present
            if (v_max is None or avg_tof is None) and hasattr(self, "beams") and self.beams:
                ions = [b for b in self.beams if b.get("massa", 136.2) >= 0.1]
                target_beams = ions if ions else self.beams
                
                velocities = []
                for b in target_beams:
                    energy = b.get("energy", 1000.0)
                    mass = b.get("massa", 136.2)
                    if mass > 0:
                        v = 1.389e4 * np.sqrt(energy / mass)
                        velocities.append(v)
                ...
                if velocities:
                    ...
                    z_starts = [b.get("orig_z", b.get("z_start", 0.0)) for b in target_beams]
                    z_start_min = min(z_starts) if z_starts else 0.0
                    L_prop = zmax - z_start_min
                    if L_prop <= 0 or L_prop >= zmax:
                        L_prop = zmax
                    
                    avg_tof = L_prop / v_min if v_min > 0 else 1e-5
            
            if v_max is None or v_max <= 0:
                v_max = 3.8e4
            if avg_tof is None or avg_tof <= 0:
                avg_tof = 1e-5
            ...
            # Only show apply button if current inputs differ from suggestions
            try:
                current_dt = float(self.txt_dt.text())
                current_T = float(self.txt_tfinal.text())
                if np.isclose(current_dt, suggested_dt, rtol=1e-2) and np.isclose(current_T, suggested_T, rtol=1e-2):
                    self.btn_apply_pic_suggestion.setVisible(False)
                else:
    ...
    def carregar_projeto_do_caminho(self, file_path, silencioso=False):
        try:
            ...
            if "domain_type" in project_data:
                dtype = project_data["domain_type"]
                if dtype == "2D_CYL":
                    self.cb_domain_type.setCurrentIndex(1)
                elif dtype == "2D":
                    self.cb_domain_type.setCurrentIndex(2)
                elif dtype == "2DCYL":
                    self.cb_domain_type.setCurrentIndex(3)
                else:
                    self.cb_domain_type.setCurrentIndex(0)
...
if __name__ == "__main__":
    ...
    # Configura a fonte global padrão do app para escalabilidade High-DPI
    from PySide6.QtGui import QFont
    global_font = QFont("Segoe UI", 10)
    global_font.setStyleHint(QFont.SansSerif)
    ...
    if os.path.exists(icon_path):
        ...
        splash.show()
        splash.showMessage(
            "IBSimion v2.0.1.e4l - Inicializando componentes...",

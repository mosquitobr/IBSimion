#!/bin/bash
# =============================================================================
# IBSimion 2.0.1.e4l - Launcher & Installer (Pipeline Automatizado)
...
echo -e "${CYAN}================================================================${NC}"
echo -e "${GREEN}    IBSimion v2.0.1.e4l - Installer & Launcher (Versão de Produção E4)   ${NC}"
echo -e "${CYAN}================================================================${NC}"
...
pkill -f "python3.*E/E4/frontend/main.py" || true
pkill -f "python3.*E/E4/frontend/splash.py" || true
pkill -f "ibsimu_wrapper" || true
if [ -f "$SCRIPT_DIR/early_splash.pid" ]; then
    rm -f "$SCRIPT_DIR/early_splash.pid"
fi

# Clean temp files, media rendering cache, and compiler garbage under E4
echo -e "  * Removendo resíduos temporários de renderizações e simulações anteriores..."
find "$SCRIPT_DIR" -type f \( -name "*.tmp" -o -name "*.log" -o -name "pout_*.txt" -o -name "field.txt" -o -name "potential.txt" -o -name "charge_density.dat" -o -name "potential_field.dat" -o -name "current_density.dat" -o -name "trajectory_density.dat" -o -name "tof.txt" -o -name "tof_histo.txt" -o -name "tof_out.txt" -o -name "trajectories.txt" -o -name "geometry.obj" -o -name "config_scenario.json" -o -name "*.mp4" -o -name "*.gif" -o -name "frame_*.png" -o -name "snapshot_*.png" -o -name "pout_*.png" \) -delete 2>/dev/null || true
...
echo -e "  * Installing required python packages (PySide6, PyVistaQt, SciPy, NumPy, EzDXF, ImageIO)...${NC}"
pip install PySide6 ezdxf pyvistaqt scipy numpy matplotlib pyvista trame imageio imageio-ffmpeg

# Se houver um arquivo requirements.txt no diretório, anexa de forma persistente
if [ -f "requirements.txt" ]; then
    echo -e "  * Anexando dependências adicionais em requirements.txt..."
    for pkg in imageio imageio-ffmpeg; do
...
echo -e "\n${GREEN}[5/5] Launching IBSimion v2.0.1.e4l UI...${NC}"

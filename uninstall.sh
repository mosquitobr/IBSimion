#!/bin/bash
# =============================================================================
# IBSimion 2.0.1.e4l - Uninstaller
...
echo -e "${CYAN}================================================================${NC}"
echo -e "${GREEN}    IBSimion v2.0.1.e4l - Uninstaller (Versão de Produção E4)   ${NC}"
echo -e "${CYAN}================================================================${NC}"
...
echo -e "  * Encerrando processos ativos do IBSimion..."
...
# 3. Remove local caches, log directories, and transient files
echo -e "\n${BLUE}[3/4] Removendo pastas de logs, caches (__pycache__, .pytest_cache)...${NC}"
find "$SCRIPT_DIR" -type d -name "__pycache__" -exec rm -rf {} + 2>/dev/null || true
find "$SCRIPT_DIR" -type d -name ".pytest_cache" -exec rm -rf {} + 2>/dev/null || true
if [ -d "logs" ]; then
    rm -rf logs
    echo -e "  * Pasta de logs removida."
fi
echo -e "  * Caches e pastas transientes limpos."
